...
class mmExtensionWizard(models.TransientModel):
    _name = "mm.extension.wizard"
    ...
    def action_extension(self):
        if self.ramping:
            ...
            if entry:
                ...
                self.letter_of_guarantee_id.is_extension = True
                self.letter_of_guarantee_id.is_ramping = True
                self.letter_of_guarantee_id.state = 'extension'
            ...
            entry2 = self.env['account.move'].create(vals)
            if entry2:
                lst = []
                val = (0, 0, {
                    'account_id': self.letter_of_guarantee_id.journal_id.expense_account_id.id,
                ...
                val2 = {
                    ...
                    'end_date': self.end_date,
                    'ramping': self.ramping,
                    'status': self.letter_of_guarantee_id.status,
                    'move_id': entry2.id,
                }
                lines.append((0, 0, val2))
                self.letter_of_guarantee_id.extension_ids = lines
                self.letter_of_guarantee_id.is_extension = True
                self.letter_of_guarantee_id.state = 'extension'
                self.letter_of_guarantee_id.move_ids = [(4, entry2.id)]
                entry2.letter_of_guarantee_id = self.letter_of_guarantee_id.id
